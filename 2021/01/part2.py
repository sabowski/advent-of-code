...
with open('input.txt') as fd:
    for line in fd:
        depth_measures.append(int(line.rstrip()))

for num, _ in enumerate(depth_measures):
    try:
        depth_measures[num+2]
        if prev_meas != None:
            if sum(depth_measures[num:num+3]) > prev_meas:
                inc_count += 1
        prev_meas = sum(depth_measures[num:num+3])
    except IndexError:
        break

print(f"total increases: {inc_count}")
