...
for rating in ["oxygen", "co2"]:
    ...
    while len(binary_numbers) > 1:
        ...
        for value in binary_numbers:
            ...
            if int(value[curr_digit]) == 1:
                running_total += 1
            else:
                running_total -= 1

        if rating == "oxygen":
            binary_numbers = [x for x in binary_numbers if x[curr_digit] == ('1' if running_total >= 0 else '0')]
        elif rating == "co2":
            binary_numbers = [x for x in binary_numbers if x[curr_digit] == ('0' if running_total >= 0 else '1')]

        curr_digit += 1

    ratings[rating] = int(binary_numbers[0],2)

print(f"Answer: {ratings['oxygen'] * ratings['co2']}")
