...
func loadData(leftList *[]int, rightList *[]int) {
	scanner := bufio.NewScanner(os.Stdin)
	for scanner.Scan() {
		data := strings.Fields(scanner.Text())
		leftData, err := strconv.Atoi(data[0])
		if err != nil {
			panic(err)
		}
		rightData, err := strconv.Atoi(data[1])
		if err != nil {
			panic(err)
		}
		*leftList = append(*leftList, leftData)
		*rightList = append(*rightList, rightData)
	}
}

func prepData(leftList *[]int, rightList *[]int) {
	sort.Ints(*leftList)
	sort.Ints(*rightList)
}
...
func part1(leftList *[]int, rightList *[]int) {
	...
	for i := range *leftList {
		total += math.Abs(float64((*leftList)[i] - (*rightList)[i]))
	}
	fmt.Println("Day 01, part 01 total:", fmt.Sprintf("%.0f", total))

}

// TODO: not sure I completely understand what is going on here
func elemCount[T any](slice []T, f func(T) bool) int {
	count := 0
	for _, s := range slice {
		if f(s) {
			count++
...
func part2(leftList *[]int, rightList *[]int) {
	...
	fmt.Println("Day 01, part 02 total:", total)
}

func main() {
	leftList := []int{}
	rightList := []int{}
	loadData(&leftList, &rightList)

	prepData(&leftList, &rightList)
