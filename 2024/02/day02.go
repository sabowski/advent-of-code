...
func checkStatus(levels []int) bool {
	...
	for i < len(levels)-1 {
		if i == 0 {
			prevCheck = levels[i+1] - levels[i]
		} else {
			currCheck := levels[i+1] - levels[i]
			if prevCheck*currCheck < 0 {
				return false
			}
			prevCheck = currCheck
		}

		if Abs(prevCheck) < 1 || Abs(prevCheck) > 3 {
			return false
		}
		i++
	}
...
func main() {
	...
	for scanner.Scan() {
		...
		if checkStatus(report) {
			safeReports++
			safeDampenedReports++
		} else {
			for i := 0; i < len(report); i++ {
				dampenedLevels := removeSliceElement(report, i)
				if checkStatus(dampenedLevels) {
					safeDampenedReports++
					break
				}
			}
	...
	fmt.Println("Part 2:", safeDampenedReports, "safe reports")
}
