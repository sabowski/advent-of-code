...
with open('input.txt') as fd:
    for line in fd:
        ...
        z1= set(x).issubset(set(y))
        z2 = set(y).issubset(set(x))
        if True in (z1, z2):
            running_total += 1

        # alternate method that I used before I realized what was wrong with my
        # first try (needing to add +1 to range). RTFM!!
        # a, b = line.strip().split(',')
        # x1 = a.split('-')
        # x = [int(i) for i in x1]
        # y1 = b.split('-')
