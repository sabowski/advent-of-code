...
with open('input.txt') as fd:
    for line in fd:
        one = line[slice(0,len(line)//2)]
        two = line[slice(len(line)//2, len(line))].strip()
        common = list(set(one)&set(two))[0]
        if common.isupper():
            running_total += ord(common) - 38
        if common.islower():
            running_total += ord(common) - 96
