...
func get_hand_type(hand string, part string) int {
	seen := make(map[byte]int)

	for i := range hand {
		if _, ok := seen[hand[i]]; !ok {
			seen[hand[i]] = 0
		}
		seen[hand[i]]++
	}
...
func sort_hands(i, j hand, part string) bool {
	var first, second int = 0, 0

	for x := 0; x < len(i.cards); x++ {
		if unicode.IsDigit(rune(i.cards[x])) {
			first, _ = strconv.Atoi(string(i.cards[x]))
		} else {
			first = card_to_int(rune(i.cards[x]), part)
		}
		if unicode.IsDigit(rune(j.cards[x])) {
			second, _ = strconv.Atoi(string(j.cards[x]))
		} else {
			second = card_to_int(rune(j.cards[x]), part)
		}
...
func card_to_int(card rune, part string) int {
	card_num := 0
	...
	case 'T':
		card_num = 10
	default:
		panic("card type that doesn't exist")
	}
	return card_num
}

func calculate_score(hands [num_types][]hand) int {
	card_no := 0
	score := 0

	for i := 0; i < num_types; i++ {
		for hand := range hands[i] {
			card_no++
			score += hands[i][hand].bid * card_no
		}
	}
	return score
}

const num_types int = 7

func main() {
	sorted_hands := [num_types][]hand{}
	p := fmt.Println
	part := os.Args[1]
	...
	for scanner.Scan() {
		...
		hand := hand{cards: line[0], bid: bid}
		hand_type := get_hand_type(hand.cards, part)
		sorted_hands[hand_type] = append(sorted_hands[hand_type], hand)
	}

	for x := 0; x < 7; x++ {
		sort.Slice(sorted_hands[x], func(i, j int) bool { return sort_hands(sorted_hands[x][i], sorted_hands[x][j], part) })
	}

	p(calculate_score(sorted_hands))
}
