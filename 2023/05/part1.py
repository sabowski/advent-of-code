...
for line in sys.stdin:
    ...
    # empty line, also the end of each conversion map
    if not line.rstrip():
        ...
        for src in srcs:
            dest_found = False
            for map in maps:
                src_range = range(map[1], map[1]+map[2])
                if src in src_range:
                    # print(f'source {src} is covered in this map', end='')
                    dest = map[0] + (src - map[1])
            ...
            if not dest_found:
                # print(f'source {src} not covered by any map, dest is {src}')
                dests.append(src)

        maps = []
        srcs = dests

    # line that starts a conversion map
    if line[0].isalpha():
        # only the first line starts with 'seeds'
        if line.startswith('seeds'):
            # no conversion for this one, so just set the seeds as dest
            dests = [int(i) for i in line.split(':')[1].split()]
        else:
            pass

    if line[0].isnumeric():
        maps.append([int(i) for i in line.split()])
