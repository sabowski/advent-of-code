...
func part1() []int {
	...
	// part 1 data
	time_a := []int{47, 70, 75, 66}
	dist_a := []int{282, 1079, 1147, 1062}

	return get_winners(time_a, dist_a)
}
...
func part2() []int {
	...
	// part 2
	time_a := []int{47707566}
	dist_a := []int{282107911471062}

	return get_winners(time_a, dist_a)
}

func get_winners(time_a, dist_a []int) []int {
	ways_to_win := make([]int, len(time_a))

	for i := 0; i < len(time_a); i++ {
		time := time_a[i]
		dist := dist_a[i]
		ways_to_win[i] = 0
...
func main() {
	part1 := part1()
	total := part1[0]
	for x := 1; x < len(part1); x++ {
		total *= part1[x]
	}
	fmt.Println("part 1:", total)
	fmt.Println("part 2:", part2()[0])
}
