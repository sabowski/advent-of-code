...
func get_data(history *[][]int) {
	...
	for scanner.Scan() {
		...
		*history = append(*history, []int{})
		...
		for i := range line {
			dp, _ := strconv.Atoi(line[i])
			(*history)[num] = append((*history)[num], dp)
		}
	}
}

func check_for_zero(vals *[]int) bool {
...
func get_exp_values(values []int) (int, int) {
	res := [][]int{}
	diffs := values
	...
	for {
		subs := []int{}
		for value := 0; value < len(diffs)-1; value++ {
			subs = append(subs, diffs[value+1]-diffs[value])
		}
		diffs = subs
		res = append(res, subs)
		if check_for_zero(&subs) {
			break
		}
	}
...
func main() {
	...
	total_prev, total_next := 0, 0
	for line := 0; line < len(history); line++ {
		next, prev := get_exp_values(history[line])
		total_prev += prev
		total_next += next
	}
	fmt.Println("Part 1:", total_next)
	fmt.Println("Part 2:", total_prev)
}
